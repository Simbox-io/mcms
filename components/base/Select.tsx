...
export default function Select({
  ...
  const [selected, setSelected] = useState<Option | Option[]>(
    value ? (isMulti ? options.filter(option => (value as string[]).includes(option.value)) : options.find(option => option.value === value)!) : (isMulti ? [] : options[0])
  );

  const handleChange = (selectedOption: Option | Option[]) => {
    setSelected(selectedOption);
    onChange(isMulti ? (selectedOption as Option[]).map(option => option.value) : (selectedOption as Option).value);
  };
      ...
      {({ open }) => (
        <>
          ...
          <div className={`relative mt-2 ${className}`}>
            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white dark:bg-gray-800 py-1.5 pl-3 pr-10 text-left text-gray-900 dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-600 sm:text-sm sm:leading-6">
