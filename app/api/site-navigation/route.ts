...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const items = await Promise.all(
      menus.map(async (menu) => {
        ...
        return menuItems.map(item => ({
          ...
          hasChildren: false // This will be updated below
        }));
      })
    );
    ...
    const itemsWithChildrenFlags = flattenedItems.map(item => {
      const hasChildren = flattenedItems.some(child => child.parentId === item.id);
      return {
        ...item,
        hasChildren
      };
    ...
    return NextResponse.json(itemsWithChildrenFlags);
  } catch (error) {
    console.error('Error fetching site navigation items:', error);
    return NextResponse.json(
      { message: 'Error fetching site navigation items' },
      { status: 500 }
    );
  }
}
